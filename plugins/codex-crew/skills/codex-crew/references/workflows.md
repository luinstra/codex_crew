...
Use when the user asks `crew code search: query` or wants the old `/crew:code-search` behavior.

1. Search the codebase directly with fast local tools such as `rg` and `rg --files`.
2. Try multiple names and likely locations when the first search is too narrow.
3. Return relevant files with line references and a concise answer.
4. If the user explicitly asks for subagents or names `crew_reader`, delegate to `crew_reader` when installed.

## Configure Workflow

Use when the user asks `crew configure` or for the old `/crew:crew-config` behavior.

Codex does not need a direct clone of Claude's `crew-config` command. The replacement is project guidance:

1. For the current project, create or update `AGENTS.md` using the Deepinit workflow.
2. Prefer project-local `AGENTS.md` over global configuration.
3. Before changing global Codex guidance, ask the user explicitly because it affects unrelated projects.
4. If the user only needs install setup, point them to the plugin README and optional custom-agent installer.
...
Use when the user asks `crew cancel build`.
...
Then summarize the current state and stop the build-loop workflow.
...
## Cancel Measure-Twice Loop

Use when the user asks `crew cancel measure twice`.
...
Then summarize any current plan state and stop the measure-twice workflow.
